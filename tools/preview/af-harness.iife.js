(function() {
	...
	*/
	...
	//#endregion
	//#region src/dev/preview-host.ts
	...
	function preseed() {
		localStorage.setItem("af-fleet-names", JSON.stringify({
			xiaoshuai: "小帅",
			xinghuo: "星火"
		}));
		localStorage.setItem("af-fleet-agents", JSON.stringify([{
			name: "新同事",
			workspace: ""
		}]));
	...
	function fakeCtx() {
		...
		const snap = (bots, color) => ({
			...
			value: {
				...
				bots: bots.map((b) => ({
					...
					bot: {
						name: b.name ?? "飞书机器人",
						avatarUrl: "data:image/svg+xml;charset=utf-8," + encodeURIComponent("<svg xmlns='http://www.w3.org/2000/svg' width='64' height='64'><rect width='64' height='64' rx='32' fill='" + color + "'/></svg>")
					}
				}))
			}
		});
		return {
			...
			connection: { rpc: { call: async (ch, endpoint, payload) => {
				...
				if (entry) {
					if (endpoint === "provision.begin") return {
						ok: true,
						value: {
							...
							durationMs: 3e5,
							qrCodeDataUrl: "data:image/svg+xml;base64," + btoa(QR_SVG),
							verificationUrl: ""
						}
					};
					if (endpoint === "provision.cancel" || endpoint === "bot.workspace.set" || endpoint === "bot.delete") return {
						ok: true,
						value: {}
					};
	...
	async function boot() {
		preseed();
		A.__ModuleLoader__ = { load({ factory }) {
			A.__AF = factory((name) => {
				if (name === "react") return import_react;
				if (name === "react-dom" || name === "react-dom/client") return import_client;
		...
		await new Promise((resolve, reject) => {
			...
			s.src = "/lib/client.js";
			s.onload = () => resolve();
			s.onerror = () => reject(/* @__PURE__ */ new Error("lib/client.js 加载失败"));
			document.head.appendChild(s);
		});
		A.__AF.apply(fakeCtx());
		const registered = A.__AF_REG;
		if (!registered) throw new Error("slots.register 未触发");
		import_client.createRoot(document.getElementById("app")).render(import_react.createElement(registered.Comp));
		if (location.search.includes("light")) document.body.classList.add("light");
	}
	boot().catch((e) => {
		const el = document.getElementById("app");
		if (el) el.textContent = "harness error: " + String(e?.message ?? e);
	});
	//#endregion
})();
