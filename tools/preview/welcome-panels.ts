...
const hostName = { zh: "小帅", en: "Xiaoshuai" };
const idxs = [0, 1, 2];
const panels = langs.map((lang) => segs.map((seg) => idxs.map((idx) => {
  ...
  return '<div class="pv-stage" id="pv-' + lang + '-' + seg + '-' + idx + '"><div class="wb-modal"><div class="wb-backdrop"></div>'
    ...
    + '<div class="wb-copy"><div class="wb-host"><div class="wb-host-avatar">' + esc(hostName[lang].slice(0, 1)) + '</div><div class="wb-host-name">' + esc(hostName[lang]) + '</div><span class="wb-dot wb-online"></span></div><div class="wb-title">' + esc(c.t) + '</div>'
...
const bodyHtml = '<div class="pv-note">五时辰 × 三句 × 中英预览台 · 下方切换 · mock 数据</div><div class="pv-bar">' + tabs + '</div>' + panels;
