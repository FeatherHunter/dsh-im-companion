/** b3-header 挂载：会话 Header 工具槽列表位（order 30）注册呼吸点。
...
import { B3HeaderAction } from './panel'

const SLOT = 'conversation.session.header.utilities'
const ID = 'dsh-im-companion:b3-header'
...
/** 挂载 B3 呼吸点；返回卸载函数（摘注册 + 样式由装配层统一回收）。 */
export function mountB3Header(ctx: FeatureCtx): () => void {
  const noop = (): void => {}
  try {
