...
export function badgeForWorkspace(workspacePath, bots, nowMs = Date.now()) {
    ...
    const kind = kinds.includes('online') ? 'online' : kinds.includes('warn') ? 'warn' : 'offline';
    const parts = bound.map((b) => {
        if (b.stale)
            return channelLabel(b.channel) + ' · 未知（轮询失败）';
