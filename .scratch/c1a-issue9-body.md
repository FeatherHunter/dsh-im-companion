...
## 修复 v4（真机反馈第四轮 · 抽屉底部越界；用户裁定：直接实现，不做原型）

反馈：整个抽屉的底部不能超过所在的设置面板的底部；当前会直接到整个DSH的底部。

根因：placeSheetPanel 用 .af-root（插件面板内容元素）原始矩形反推四边，而 .af-root 高度 = Agent 列表内容高度。列表高于设置面板可视区时，viewport.height - panel.bottom 为负，由 sheetGeometry 的 Math.max(0) 钳成 0 → 抽屉 bottom:0 → 顶到视口（DSH）底。列表矮时恰好贴着面板底，此前真机未暴露。

修复（commit 2d3f590，3 文件 +70/-8，共享层零触碰）：
- src/features/c1a/data.ts：新增纯函数 clipRect（元素矩形 × 裁剪框序列求交；失效回 null，DOM-free 可直测）。
- src/features/c1a/position.ts：panelRect 改为 .af-root 可见矩形——沿祖先链收集裁剪框（overflow 非 visible 容器的 padding-box，即设置面板内容滚动区 / 弹窗），与原矩形求交；顶/底/左右全部锁在面板可见区内；裁没回 null → 既有 CSS 兜底；resize 跟随保持。
- tools/verify/features/c1a-drawer.ts：新增用例「可见矩形裁剪——内容高于面板时底边锁面板底」：断言旧路径 bottom=0（翻车复现）vs 新路径裁剪后 bottom=视口高-面板底；另覆盖顶边夹取、无裁剪原样、完全裁没回 null。

证据：
- npm run build PASS（host + client，lib/client.js 已重建）。
- guard PASS（71 文件，position.ts 66 / data.ts 220 / c1a 各文件 <300）。
- 几何断言 5/5（.scratch/c1a-geometry-check.cjs：最小依赖集独立转译 c1a/data.ts 后直跑同断言，绕开树级转译红）。
- 树级红声明：src/features/left-badges/hover-card.ts 272/274 UIEvent 类型错（并行会话，与 2026-09-03 阻塞声明一致），连带 npm run typecheck 与 c1a-drawer 整链 verify 无法转译；c1a 自身文件 typecheck 零错误。

待确认（close 后验收步骤；如不通过请 reopen 或另开跟进）：
① 完全退出 DSH Desktop 重开 + Ctrl+F5；
② 设置 → IM机器人辅助 → 滚动使内容超出面板 → 点行内「详情」→ 抽屉底边不超过设置面板底边（不顶到 DSH 窗口底），顶边不高于面板顶；
③ 窗口缩放后抽屉仍贴面板（resize 跟随）。

## 进度：100%

下一步：用户裁定任务完成，已按指示关闭；close 后解锁 E3（#14）。真机验收步骤见「修复 v4 · 待确认」，如不通过 reopen 或另开跟进单。

待确认：见「修复 v4 · 待确认」三项验收步骤；历史待确认项（真机验收主流程 / Spec 复审 / dsh-im 上游 research）保持已记账，验收按修复 v4 清单为准。
