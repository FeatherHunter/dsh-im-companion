...
## 方向变更（2026-09-04，用户裁定）

家不是一次性用品：永别概念（含一切“再也不见”暗示）全删；按钮是“进门”不是“永别”。B 全家福 / F 授勋彻底删除；E 由“第一天”重构为“纪念日”（记住相遇，家一直在）。原型进入第三轮五方向（A/C/E/G/H，提交 9e3179b），五变体回归探针全过。

影响：一次性持久化（meta.seen）暂冻结——家是常驻地，出现规则回到“空会话即家”（可见性门保留）；未绑定零 UI 不变。

## 进度：95%

下一步：请在 A/C/E/G/H 里 verdict（可拼），然后我按赢家重写实现（当前 src 实现仍是旧 A 变体欢迎横幅 + 内存关闭，需按新 verdict 重做出现/按钮语义）。

待确认：① 五选一 verdict；② 旧 3 处偏离随新方向作废与否。以上未确认前不 close、不重写实现。R3 修订改由 #14 跟踪。
